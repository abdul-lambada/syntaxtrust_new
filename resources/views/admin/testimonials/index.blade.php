                        ...
                        <tbody>
                            @forelse($items as $i => $s)
                                <tr>
                                    ...
                                    <td>
                                        @if($s->avatar_url)
                                            <img src="{{ $s->avatar_url }}" alt="avatar" style="height:34px;width:34px" class="rounded-circle border">
                                        @else
                                            <div class="rounded-circle bg-secondary text-white d-inline-flex align-items-center justify-content-center" style="height:34px;width:34px">{{ strtoupper(substr($s->author_name,0,1)) }}</div>
                                        @endif
                                    </td>
                                    <td>{{ $s->author_name }}</td>
                                    <td>{{ $s->author_role }}</td>
                                    ...
                                    <td>
                                        <a href="{{ route('admin.testimonials.edit', $s) }}" class="btn btn-sm btn-outline-primary">Edit</a>
                                        <button type="button" class="btn btn-sm btn-outline-danger" data-bs-toggle="modal" data-bs-target="#modalDelete" data-id="{{ $s->id }}" data-name="{{ $s->author_name }}">Hapus</button>
                                    </td>
                                </tr>
                            @empty
...
<div class="modal fade" id="modalDelete" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog"><div class="modal-content">
      <div class="modal-header"><h5 class="modal-title">Hapus Testimoni</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
