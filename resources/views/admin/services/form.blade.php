                ...
                <form method="POST" enctype="multipart/form-data" action="{{ $service->exists ? route('admin.services.update',$service) : route('admin.services.store') }}">
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="text" name="slug" value="{{ old('slug', $service->slug) }}" class="form-control">
                            @error('slug')<div class="text-danger small">{{ $message }}</div>@enderror
                        </div>
                        <div class="col-12">
                            <label class="form-label">Excerpt</label>
                            <input type="text" name="excerpt" value="{{ old('excerpt', $service->excerpt) }}" class="form-control">
                        ...
                        <div class="col-md-4">
                            <label class="form-label">Icon (teks/URL)</label>
                            <input type="text" name="icon" value="{{ old('icon', $service->icon) }}" class="form-control" placeholder="nama-icon atau URL (opsional)">
                            @error('icon')<div class="text-danger small">{{ $message }}</div>@enderror
                            <div class="form-text">Atau unggah file di bawah ini.</div>
                            <input type="file" name="icon_file" class="form-control mt-2" accept="image/*">
                            @error('icon_file')<div class="text-danger small">{{ $message }}</div>@enderror
                            @if($service->icon)
                                <div class="mt-2">
                                    <img src="{{ $service->icon }}" alt="icon" style="height:40px">
                                </div>
                            @endif
                            <div class="form-check mt-2">
                                <input class="form-check-input" type="checkbox" name="remove_icon" id="remove_icon" value="1">
                                <label class="form-check-label" for="remove_icon">Hapus gambar</label>
                            </div>
                        </div>
                        <div class="col-md-4">
