...
@section('content')
    ...
    <!-- Hero -->
    <section class="relative overflow-hidden transition-colors duration-500" x-data="heroTheme()" x-init="init()"
        :class="theme === 'night' ? 'bg-slate-950 text-white' : 'bg-neutral-50 text-neutral-900'">
        ...
        <div class="absolute top-6 right-6 z-20 flex items-center gap-2 text-xs">
            <span class="font-semibold uppercase tracking-wider"
                :class="theme === 'night' ? 'text-indigo-200' : 'text-indigo-600'">Tema</span>
            <button type="button" @click="toggle()"
                class="inline-flex items-center gap-2 rounded-full border px-3 py-1.5 backdrop-blur-sm transition"
                :class="theme === 'night' ? 'border-white/30 bg-white/10 text-white hover:bg-white/20' :
            ...
            <div class="grid lg:grid-cols-2 gap-10 items-center">
                <div data-reveal :class="theme === 'night' ? 'text-neutral-100' : ''">
                    <h1 class="text-3xl sm:text-4xl lg:text-5xl font-bold leading-tight">
                        SyntaxTrust
                        <span class="block" :class="theme === 'night' ? 'text-indigo-300' : 'text-indigo-600'">Jasa
                            Pembuatan
                            & Modifikasi Website</span>
                    </h1>
                    <p class="mt-5 text-lg" :class="theme === 'night' ? 'text-neutral-300' : 'text-neutral-600'">Kami
                        menerima
                        pembuatan website tugas kuliah, modifikasi website yang sudah ada, hingga pembuatan website dari
                        nol. Harga fleksibel dapat dinegosiasikan sesuai scope dan tingkat kerumitan.</p>
                    <div class="mt-8 flex flex-wrap gap-3">
                        <a href="#kontak"
                            class="group relative px-5 py-3 rounded-xl bg-indigo-600 text-white transition hover:bg-indigo-500 active:scale-[0.98]">
                            <span
                                class="absolute inset-0 rounded-xl bg-white/20 opacity-0 group-hover:opacity-100 transition"></span>
                            <span class="relative">Konsultasi Gratis</span>
                        </a>
                        <a href="#layanan" class="group relative px-5 py-3 rounded-xl border active:scale-[0.98]"
                            :class="theme === 'night' ? 'border-white/40 text-white hover:border-white/70' :
                                'border-neutral-300 hover:border-neutral-400'">
                            <span class="absolute inset-0 rounded-xl"
                                :class="theme === 'night' ? 'bg-white/10 opacity-0 group-hover:opacity-100 transition' :
                                    'bg-neutral-100/50 opacity-0 group-hover:opacity-100 transition'"></span>
                            <span class="relative" :class="theme === 'night' ? 'text-white' : ''">Lihat Layanan</span>
                        </a>
                    </div>
                    <div class="mt-6 text-sm" :class="theme === 'night' ? 'text-neutral-400' : 'text-neutral-500'">Waktu
                        pengerjaan cepat. Revisi sewajarnya. Dukungan pasca-deploy.</div>
                </div>
                <div class="relative" data-reveal>
                    <div class="mx-auto max-w-sm rounded-3xl border border-neutral-200/70 backdrop-blur-sm shadow-xl p-6 typewriter-shell"
                        ...
                        <div class="flex flex-wrap justify-center gap-2">
                            <template x-for="(item, i) in items" :key="i">
                                <button type="button" class="relative px-3 py-1.5 rounded-full text-sm transition border"
                                    :class="index === i ? 'text-white shadow-lg shadow-neutral-900/20' :
                                        'bg-white/80 text-neutral-600 hover:border-neutral-300'"
                                    :style="index === i ? `background:${item.accent}; border-color:${item.accent};` :
                                        'border-color:rgba(148,163,184,0.5);'"
                                    @click="select(i)" x-text="item.label"></button>
                            </template>
                        </div>

                        <div class="mt-6 border border-neutral-200/70 rounded-2xl bg-white/95">
                            <div
                                class="flex items-center gap-1 px-5 pt-4 pb-3 text-[10px] font-semibold tracking-[0.35em] text-neutral-400 uppercase">
                            ...
                            <div class="px-5 pb-6">
                                <div class="flex items-center gap-3">
                                    <div class="h-12 w-12 rounded-2xl bg-neutral-900 text-white grid place-items-center text-2xl shadow-inner shadow-neutral-900/40"
                                        x-text="current?.emoji"></div>
                                    <div>
                                        <div class="font-semibold typewriter-title" x-text="current?.title"></div>
                                        <div class="text-xs uppercase tracking-wide typewriter-tagline"
                                            x-text="current?.tagline"></div>
                                    </div>
                                </div>
                                <div class="mt-5 font-mono text-[1.1rem] leading-relaxed typewriter-typed min-h-14">
                                    <span x-text="displayText"></span>
                                    <span class="inline-block w-2 translate-y-0.5 border-l-2 border-indigo-500"
                                        :class="caretVisible ? 'opacity-100' : 'opacity-0'"></span>
                                </div>
                        ...
                        <div class="mt-6 flex justify-center gap-3">
                            <button type="button"
                                class="h-9 w-9 rounded-full border border-neutral-200/80 bg-white text-neutral-500 hover:text-neutral-900 hover:border-neutral-300 transition"
                                @click="prev()">
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none"
                                    stroke="currentColor" class="h-4 w-4 mx-auto">
        ...
        <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
            ...
            <div class="mt-8 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
                ...
                <div class="rounded-2xl border border-neutral-200 p-6 transition hover:shadow-lg hover:-translate-y-0.5">
                    ...
                    <p class="mt-1 text-sm text-neutral-600">Estimasi dan scope jelas. Harga bisa dinego sesuai kebutuhan.
                    </p>
                </div>
                <div class="rounded-2xl border border-neutral-200 p-6 transition hover:shadow-lg hover:-translate-y-0.5">
                    <div class="h-10 w-10 rounded-lg bg-neutral-900 text-white grid place-items-center">
                ...
                <div class="rounded-2xl border border-neutral-200 p-6 transition hover:shadow-lg hover:-translate-y-0.5">
                    ...
                    <p class="mt-1 text-sm text-neutral-600">Laravel 12, Tailwind v4, Vite, dan Alpine untuk interaksi
                        cepat.</p>
                </div>
            </div>
        </div>
    ...
    <section id="pricing" class="py-20 bg-neutral-50/30">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="text-center mb-16" data-reveal>
                <h2 class="text-3xl font-bold text-neutral-900 border-b-4 border-indigo-600 inline-block pb-2">Investasi
                    Layanan</h2>
                <p class="mt-4 text-neutral-600">Terima pembuatan dari nol, modifikasi website, hingga tugas kuliah. Harga
                    fleksibel & dapat dinegosiasikan.</p>
                <p class="text-xs text-neutral-400 mt-2">* Harga di bawah belum termasuk biaya hosting & domain (opsional).
                </p>
            </div>

            <div class="grid md:grid-cols-3 gap-8 items-stretch pt-4">
                @foreach ($packages as $pkg)
                    <div class="relative group rounded-3xl border {{ $pkg->is_highlighted ? 'border-2 border-indigo-600 shadow-2xl md:scale-105 z-10' : 'border-neutral-200 shadow-sm' }} bg-white p-8 transition-all duration-300 hover:shadow-2xl hover:-translate-y-2 flex flex-col"
                        data-reveal>
                        @if ($pkg->badge)
                            <div
                                class="absolute -top-4 left-1/2 -translate-x-1/2 bg-indigo-600 text-white text-[10px] font-bold uppercase tracking-widest px-6 py-1.5 rounded-full shadow-lg">
                                {{ $pkg->badge }}</div>
                        @endif

                        <div class="mb-8">
                            <h3 class="{{ $pkg->is_highlighted ? 'text-2xl' : 'text-xl' }} font-bold text-neutral-900">
                                {{ $pkg->name }}</h3>
                            <p class="text-sm text-neutral-500 mt-1">{{ $pkg->tagline }}</p>
                            <div class="mt-6 flex items-baseline gap-1">
                                <span class="text-sm font-semibold text-neutral-500">Mulai</span>
                                <span
                                    class="{{ $pkg->is_highlighted ? 'text-5xl font-extrabold text-indigo-600' : 'text-4xl font-bold text-neutral-900' }} tracking-tight">{{ $pkg->price_text }}</span>
                                @if ($pkg->price_subtext)
                                    <span class="text-sm text-neutral-400 font-medium">{{ $pkg->price_subtext }}</span>
                                @endif
                            </div>
                        </div>
                        ...
                        <ul class="space-y-4 mb-10 grow">
                            @foreach ($pkg->features as $feature)
                                <li
                                    class="flex items-start gap-3 text-sm {{ $feature['active'] ?? true ? 'text-neutral-600' : 'text-neutral-400 line-through' }}">
                                    @if ($feature['active'] ?? true)
                                        <svg xmlns="http://www.w3.org/2000/svg"
                                            class="h-5 w-5 {{ $pkg->is_highlighted ? 'text-indigo-600' : 'text-emerald-500' }} shrink-0"
                                            fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M5 13l4 4L19 7" />
                                        </svg>
                                    @else
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-neutral-300 shrink-0"
                                            fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        <a href="https://wa.me/{{ preg_replace('/\D/', '', $contacts->where('type', 'whatsapp')->first()->value ?? '6285156553226') }}?text={{ urlencode($pkg->whatsapp_message ?? 'Halo SyntaxTrust, saya tertarik dengan Paket ' . $pkg->name) }}"
                            class="block text-center py-4 px-6 rounded-2xl {{ $pkg->is_highlighted ? 'bg-indigo-600 hover:bg-indigo-700 shadow-xl shadow-indigo-200' : 'bg-neutral-900 hover:bg-neutral-800 shadow-lg' }} text-white font-bold transition-all">
                            {{ $pkg->is_highlighted ? 'Ambil Promonya' : ($pkg->price_text === 'Hubungi' ? 'Konsultasi Dulu' : 'Pilih Paket') }}
                        </a>
                    </div>
                @endforeach
            </div>
            ...
            <section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
                ...
                <div class="mt-8 grid gap-6 md:grid-cols-4">
                    @foreach ($process ?? [] as $idx => $ps)
                        <div x-data="{ h: false }" @mouseenter="h=true" @mouseleave="h=false"
                            class="rounded-2xl border border-neutral-200 p-6 transition"
                            :class="h ? 'shadow-lg ring-1 ring-indigo-200' : ''">
            ...
            <section class="bg-white">
                <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16" x-data="{
                    ...
                    <p class="mt-2 text-neutral-600 text-center" data-reveal>Dari brief hingga live dengan tahapan jelas.
                    </p>

                    <div class="mt-10 relative">
                        <div class="absolute left-0 right-0 top-5 h-1 bg-neutral-200 rounded-full"></div>
                        <div class="absolute left-0 top-5 h-1 bg-indigo-600 rounded-full transition-all duration-500"
                            :style="`right: calc(100% - ${(step-1)/(steps.length-1)*100}%);`"></div>

                        <div class="relative z-10 grid grid-cols-5 gap-2 pt-8">
                            <template x-for="(s, idx) in steps" :key="idx">
                                <div class="flex flex-col items-center">
                                    <button @click="step=idx+1"
                                        class="relative z-20 h-10 w-10 rounded-full grid place-items-center border transition"
                                        :class="idx + 1 <= step ? 'bg-indigo-600 text-white border-indigo-600' :
            ...
            <section class="bg-white">
                <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-16" x-data="testimonialCarousel(@js($testimonialItems ?? []))"
                    x-init="init()">
                    <h2 class="text-2xl font-bold text-center" data-reveal>Apa Kata Klien</h2>
                    <p class="mt-2 text-neutral-600 text-center" data-reveal>Testimoni dari klien yang telah menggunakan
                        layanan
                        kami.</p>
                    <div class="mt-8 relative" x-show="items.length">
                        <div class="overflow-hidden rounded-3xl border border-neutral-200 bg-white/95 shadow-lg"
                            x-ref="panel" @mouseenter="stop()" @mouseleave="start()"
                            @touchstart.passive="sx=$event.touches[0].clientX"
                            @touchend.passive="dx=$event.changedTouches[0].clientX; if(sx-dx>40) next(); if(dx-sx>40) prev();">
                            <template x-for="(item, idx) in items" :key="idx">
                                <article :data-index="idx" x-show="index===idx" x-transition.opacity.duration.300ms
                                    class="grid gap-6 sm:grid-cols-[64px,1fr] items-start px-8 py-10 relative"
                                    x-intersect="observe($el)">
                                    <div class="hidden sm:block">
                                        <template x-if="item.avatar">
                                            <img :src="item.avatar"
                                                class="h-16 w-16 rounded-full object-cover border border-neutral-200 shadow-sm"
                                                alt="avatar">
                                        </template>
                                        <template x-if="!item.avatar">
                                            <div
                                                class="h-16 w-16 rounded-full grid place-items-center bg-indigo-600/15 text-indigo-600 font-semibold">
                                    ...
                                    <div class="relative">
                                        <div
                                            class="absolute -left-6 -top-4 h-12 w-12 rounded-full bg-indigo-500/10 text-indigo-500 grid place-items-center">
                                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"
                                                fill="currentColor" class="h-6 w-6">
                                                <path
                                                    d="M7.17 6A5.17 5.17 0 0 0 2 11.17V22h8.27v-8.27H6.1c0-2.28 1.1-3.34 3.45-4.65L7.17 6Zm10 0A5.17 5.17 0 0 0 12 11.17V22h8.27v-8.27h-4.17c0-2.28 1.1-3.34 3.45-4.65L17.17 6Z" />
                                            </svg>
                                        </div>
                                        <header class="pl-6">
                                            <h3 class="text-lg font-semibold testimonial-title" :style="titleStyle(idx)"
                                                x-text="item.title || item.name"></h3>
                                        </header>
                                        <p class="mt-5 text-lg leading-relaxed testimonial-quote" :style="quoteStyle(idx)"
                                            x-text="`“ ${item.text} ”`"></p>
                                        <footer class="mt-6 flex items-center justify-between flex-wrap gap-3 pl-6">
                                            <div class="text-sm font-medium testimonial-meta"><span
                                                    x-text="item.name"></span>
                                                <span x-show="item.role" x-text="`· ${item.role}`"></span>
                                            </div>
                                            <div class="flex items-center gap-0.5" aria-label="rating">
                                                <template x-for="r in 5" :key="r"><svg
                                                        xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"
                                                        class="h-4 w-4"
                                                        :class="r <= item.rating ? 'text-amber-500 fill-current' :
                                                            'text-neutral-300 fill-current'">
                                                        <path
                                                            d="M12 .587l3.668 7.431L24 9.753l-6 5.848 1.417 8.268L12 19.771l-7.417 4.098L6 15.601 0 9.753l8.332-1.735z" />
                                                    </svg></template>
                                            </div>
                                        </footer>
                                    </div>
                        ...
                        <button type="button"
                            class="absolute left-0 top-1/2 -translate-y-1/2 m-3 h-10 w-10 rounded-full border border-neutral-200 bg-white shadow hover:bg-neutral-50 hidden sm:flex items-center justify-center"
                            @click="prev()">
                        ...
                        <button type="button"
                            class="absolute right-0 top-1/2 -translate-y-1/2 m-3 h-10 w-10 rounded-full border border-neutral-200 bg-white shadow hover:bg-neutral-50 hidden sm:flex items-center justify-center"
                            @click="next()">
                            <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" viewBox="0 0 24 24" fill="none"
                                stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                        ...
                        <div class="mt-12 grid gap-4 sm:grid-cols-3" x-show="items.length">
                            <template x-for="(item, idx) in items" :key="`thumb-${idx}`">
                                <button type="button"
                                    class="testimonial-thumb rounded-2xl p-4 text-left flex items-center gap-4"
                                    :class="index === idx ? 'testimonial-thumb-active' : ''" @click="go(idx)">
                                    <div class="h-12 w-12 rounded-full overflow-hidden border border-current/20 shrink-0">
                                        <template x-if="item.avatar">
                                            <img :src="item.avatar" class="h-full w-full object-cover"
                                                alt="avatar">
                                        </template>
                                        <template x-if="!item.avatar">
                                            <div class="h-full w-full grid place-items-center bg-indigo-500/15 text-indigo-500 font-semibold"
                    ...
                    <template x-if="!items.length">
                        <div
                            class="mt-8 rounded-2xl border border-dashed border-neutral-300 bg-white/70 p-10 text-center text-neutral-500">
            ...
            <section id="jadwal" class="bg-white">
                <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-16" x-data="{
                    ...
                    <p class="mt-2 text-neutral-600 text-center" data-reveal>Tentukan jadwal Anda sendiri dan kirim
                        otomatis ke
                        WhatsApp kami.</p>

                    <div class="mt-8 grid lg:grid-cols-2 gap-8">
                        <form @submit.prevent="submit()"
                            class="relative z-10 rounded-2xl border border-neutral-200 bg-white p-6 grid gap-4 scheduler-shell"
                            data-reveal>
                            <div>
                                ...
                                <div class="mt-2 h-1.5 w-full rounded-full bg-neutral-200 overflow-hidden">
                                    <div
                                        class="absolute inset-0 bg-linear-to-r from-indigo-500/5 to-violet-500/5 lg:hidden">
                                    </div>
                                    <div class="h-full bg-linear-to-r from-indigo-500 via-purple-500 to-emerald-500 transition-all duration-300"
                                        :style="`width:${progressPercent}%`"></div>
                                </div>
                                <div
                                    class="mt-2 grid grid-cols-4 gap-2 text-[0.65rem] uppercase tracking-wide text-neutral-400">
                                    <template x-for="step in stepsState" :key="step.label">
                                        <div class="flex items-center gap-1"
                                            :class="step.done ? 'text-indigo-600 font-semibold' : ''">
                                            <span class="inline-flex h-2 w-2 rounded-full"
                                                :class="step.done ? 'bg-indigo-500' : 'bg-neutral-300'"></span>
                            ...
                            <div class="grid sm:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input x-model="name" @blur="touched.name=true" type="text"
                                        ...
                                        'border-red-400 focus:ring-red-200 focus:border-red-400' : 'border-neutral-300'"
                                        required>
                                    <p x-show="touched.name && !nameValid" class="mt-1 text-xs text-red-500">Nama minimal
                                        3
                                        karakter.</p>
                                </div>
                                <div>
                                    ...
                                    <div>
                                        ...
                                        <div class="mt-2 flex flex-wrap gap-2">
                                            <button type="button" @click="preset('today-19')"
                                                class="px-2.5 py-1.5 rounded-lg border border-neutral-300 text-xs hover:bg-neutral-50">Hari
                                                ini 19:00</button>
                                            <button type="button" @click="preset('tom-10')"
                                                class="px-2.5 py-1.5 rounded-lg border border-neutral-300 text-xs hover:bg-neutral-50">Besok
                                                10:00</button>
                                            ...
                                            <div x-data="{ open: false }" class="relative">
                                                <button type="button" @click="open=!open"
                                                    class="px-2.5 py-1.5 rounded-lg border border-neutral-300 text-xs hover:bg-neutral-50">Lainnya</button>
                                                <div x-show="open" @click.outside="open=false" x-transition
                                                    class="absolute left-0 z-40 mt-1 w-44 rounded-lg border border-neutral-200 bg-white shadow-lg">
                                                    <button type="button" @click="preset('day2-10'); open=false"
                                                        class="block w-full text-left px-3 py-1.5 text-xs hover:bg-neutral-50">Lusa
                                                        10:00</button>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    <p class="mt-1 text-xs text-neutral-500">Zona waktu: <span x-text="tz"></span>.
                                        Pastikan
                                        waktu sesuai.</p>
                                    <p x-show="touched.when && !validWhen" class="mt-1 text-xs text-red-600">Tanggal &
                                        waktu tidak
                                        valid atau sudah lewat.</p>
                                </div>
                            </div>
                            <div class="grid sm:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <div class="flex gap-2">
                                        <button type="button" @click="method='meet'"
                                            ...
                                            :class="method === 'meet' ? 'bg-indigo-600 text-white border-indigo-600' :
                                                'bg-white text-neutral-700 border-neutral-300'">Google
                                            Meet</button>
                                        <button type="button" @click="method='wa'"
                                            class="px-3 py-1.5 rounded-full text-sm border transition"
                                            :class="method === 'wa' ? 'bg-indigo-600 text-white border-indigo-600' :
                                                'bg-white text-neutral-700 border-neutral-300'">WhatsApp</button>
                                    </div>
                                </div>
                                <div x-show="method==='meet'" x-transition>
                                    <label class="block text-sm mb-1">Link Google Meet (opsional)</label>
                                    <input x-model="link" @blur="touched.link=true" type="url"
                                        placeholder="https://meet.google.com/..."
                                        class="w-full rounded-lg border px-3 py-2"
                                        :class="(!validLink && touched.link) ?
                                        'border-red-400 focus:ring-red-200 focus:border-red-400' : 'border-neutral-300'">
                                    <p x-show="touched.link && !validLink" class="mt-1 text-xs text-red-600">Link Meet
                                        tidak
                                        valid. Gunakan format https://meet.google.com/xxx-xxxx-xxx</p>
                                </div>
                            </div>
                            <div>
                                <label class="block text-sm mb-1">Topik</label>
                                <input x-model="topic" @blur="touched.topic=true" type="text"
                                    placeholder="Contoh: Diskusi scope landing page"
                                    class="w-full rounded-lg border px-3 py-2"
                                    :class="(touched.topic && !topicValid) ?
                                    'border-red-400 focus:ring-red-200 focus:border-red-400' :
                                    'border-neutral-300'"
                                    required>
                                <p x-show="touched.topic && !topicValid" class="mt-1 text-xs text-red-500">Tuliskan topik
                                    minimal
                                    5 karakter.</p>
                            </div>
                            <div class="flex items-center justify-between gap-3">
                                <div class="flex items-center gap-3">
                                    ...
                                    <button type="button" @click="resetAll()"
                                        class="text-xs underline text-neutral-500 hover:text-neutral-700">Reset</button>
                                </div>
                                <div class="flex items-center gap-3">
                                    <div class="text-xs text-neutral-500">WA tujuan: +<span x-text="wa"></span></div>
                                    <button type="submit" :disabled="!formValid || submitting"
                                        :class="(!formValid || submitting) ? 'opacity-60 cursor-not-allowed' : ''"
                                        class="px-5 py-2 rounded-lg bg-emerald-600 text-white hover:bg-emerald-500 transition flex items-center gap-2">
                        ...
                        <div class="relative z-0 rounded-2xl border border-neutral-200 p-6 bg-white scheduler-shell"
                            data-reveal>
                            <div class="text-sm preview-title">Pratinjau Pesan</div>
                            <div class="mt-2 rounded-lg p-4 text-sm whitespace-pre-line preview-text"
                    ...
                    <div x-show="toast" x-transition class="fixed bottom-6 left-1/2 -translate-x-1/2 z-50">
                        <div class="rounded-full bg-neutral-900 text-white px-4 py-2 text-sm shadow-lg">
                            {{ __('Terkirim. Lanjutkan di WhatsApp.') }}</div>
                    </div>
                </div>
            </section>
            ...
            <section id="kontak" class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
                <div class="grid lg:grid-cols-2 gap-8">
                    <div>
                        ...
                        <p class="mt-2 text-neutral-600">Ceritakan ide atau kebutuhan Anda, kami siap membantu dari
                            perencanaan
                            hingga eksekusi.</p>
                    ...
                    <form x-data="{ name: '', email: '', msg: '', sent: false }" @submit.prevent="sent=true"
                        class="rounded-xl border border-neutral-200 p-6 bg-white">
                        <div class="grid sm:grid-cols-2 gap-4">
                            <div>
                                <label class="block text-sm mb-1">Nama</label>
                                <input x-model="name" type="text"
                                    class="w-full rounded-lg border border-neutral-300 px-3 py-2" required>
                            </div>
                            <div>
                        ...
                        <button type="submit" :disabled="!name || !email || !msg"
                            :class="(!name || !email || !msg) ? 'opacity-50 cursor-not-allowed' : ''"
                            class="mt-4 px-4 py-2 rounded-lg bg-neutral-900 text-white transition active:scale-[0.98]">Kirim</button>
                        <p x-show="sent" x-transition class="mt-3 text-sm text-green-600">Terima kasih! Pesan Anda telah
                            terekam
                            (demo).</p>
                    </form>
                </div>
