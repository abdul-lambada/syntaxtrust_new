...
@section('content')
    ...
    <section class="bg-white">
        <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-16" x-data="testimonialCarousel(@js($testimonialItems ?? []))" x-init="init()">
            ...
            <p class="mt-2 text-neutral-600 text-center" data-reveal>Testimoni dari klien yang telah menggunakan layanan
                kami.</p>
            <div class="mt-8 relative" x-show="items.length">
                <div class="overflow-hidden rounded-3xl border border-neutral-200 bg-white/95 shadow-lg" x-ref="panel"
                    @mouseenter="stop()" @mouseleave="start()" @touchstart.passive="sx=$event.touches[0].clientX"
                        ...
                        <article :data-index="idx" x-show="index===idx" x-transition.opacity.duration.300ms
                            ...
                            <div class="relative">
                                ...
                                <footer class="mt-6 flex items-center justify-between flex-wrap gap-3 pl-6">
                                    <div class="text-sm font-medium testimonial-meta"><span x-text="item.name"></span>
                ...
                <div class="mt-12 grid gap-4 sm:grid-cols-3" x-show="items.length">
                    <template x-for="(item, idx) in items" :key="`thumb-${idx}`">
                        <button type="button" class="testimonial-thumb rounded-2xl p-4 text-left flex items-center gap-4"
    ...
    <section id="jadwal" class="bg-white">
        ...
        $watch('remember', () => save());">
            ...
            <p class="mt-2 text-neutral-600 text-center" data-reveal>Tentukan jadwal Anda sendiri dan kirim otomatis ke
                WhatsApp kami.</p>
                ...
                <form @submit.prevent="submit()"
                    ...
                    <div>
                        ...
                        <div class="mt-2 h-1.5 w-full rounded-full bg-neutral-200 overflow-hidden">
                            <div class="absolute inset-0 bg-linear-to-r from-indigo-500/5 to-violet-500/5 lg:hidden"></div>
                    ...
                    <div class="grid sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input x-model="name" @blur="touched.name=true" type="text"
                                ...
                                required>
                            <p x-show="touched.name && !nameValid" class="mt-1 text-xs text-red-500">Nama minimal 3
                                karakter.</p>
                        </div>
                        <div>
                            ...
                            <p class="mt-1 text-xs text-neutral-500">Zona waktu: <span x-text="tz"></span>. Pastikan
                                waktu sesuai.</p>
                            <p x-show="touched.when && !validWhen" class="mt-1 text-xs text-red-600">Tanggal & waktu tidak
                    ...
                    <div class="grid sm:grid-cols-2 gap-4">
                        ...
                        <div x-show="method==='meet'" x-transition>
                            ...
                            <input x-model="link" @blur="touched.link=true" type="url"
                                placeholder="https://meet.google.com/..." class="w-full rounded-lg border px-3 py-2"
                                :class="(!validLink && touched.link) ?
                                'border-red-400 focus:ring-red-200 focus:border-red-400' : 'border-neutral-300'">
                            <p x-show="touched.link && !validLink" class="mt-1 text-xs text-red-600">Link Meet tidak
                    ...
                    <div>
                        ...
                        <input x-model="topic" @blur="touched.topic=true" type="text"
                            placeholder="Contoh: Diskusi scope landing page" class="w-full rounded-lg border px-3 py-2"
                            :class="(touched.topic && !topicValid) ? 'border-red-400 focus:ring-red-200 focus:border-red-400' :
                            'border-neutral-300'"
                            required>
                        <p x-show="touched.topic && !topicValid" class="mt-1 text-xs text-red-500">Tuliskan topik minimal
                            5 karakter.</p>
                    </div>
                    <div class="flex items-center justify-between gap-3">
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <button type="submit" :disabled="!formValid || submitting"
                                ...
                                <svg x-show="submitting" xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 animate-spin"
    ...
    <section id="kontak" class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div class="grid lg:grid-cols-2 gap-8">
            <div>
                ...
                <p class="mt-2 text-neutral-600">Ceritakan ide atau kebutuhan Anda, kami siap membantu dari perencanaan
                    hingga eksekusi.</p>
            ...
            <form x-data="{ name: '', email: '', msg: '', sent: false }" @submit.prevent="sent=true"
                ...
                <p x-show="sent" x-transition class="mt-3 text-sm text-green-600">Terima kasih! Pesan Anda telah terekam
                    (demo).</p>
            </form>
        </div>
    </section>
